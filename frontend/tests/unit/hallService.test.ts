...
describe("hallService", () => {
  ...
  describe("getHall", () => {
    it("returns a single hall by id on success", async () => {
      ...
      expect(result.names[0].name).toBe("Hall A");
    });
  ...
  describe("createHall", () => {
    it("creates a new hall and returns it with id", async () => {
      ...
      expect(result.names.length).toBe(1);
      expect(result.names[0].name).toBe("New Hall");
    });
  ...
  describe("updateHall", () => {
    it("updates a hall and returns updated data", async () => {
      const hallData: HallUpdate = {
        names: [{ language: "en", name: "Updated Hall" }],
      ...
      expect(result).toEqual(mockResponse);
      expect(result.names.length).toBe(1);
      expect(result.names[0].name).toBe("Updated Hall");
