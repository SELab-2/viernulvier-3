...
const FilterSidebar: React.FC<Props> = ({
  ...
  useEffect(() => {
    const mostPopularTags = [
      ...
      "by viernulvier", "in de vooruit", "in club wintercircus", "op locatie",
      "met audiodescriptie", "relaxed performance", "met tolk vgt", "cadeaubon geldig",
    ];
  ...
  return (
    <aside ref={sidebarRef} id="archive-sidebar" className={`${show ? "block" : "hidden"} sticky-scroll mb-10 max-h-[calc(100vh-120px)] w-full space-y-6 overflow-y-auto pr-0 lg:sticky lg:top-24 lg:mb-0 lg:block lg:w-80 lg:overflow-y-auto lg:pr-4`}>
      <FilterCard title={t("filter.search")}>
        <div className="relative">
          ...
          <svg className="pointer-events-none absolute top-1/2 right-3 h-3.5 w-3.5 -translate-y-1/2 opacity-25" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <FilterCard title={t("filter.tags")}>
        <div className="space-y-4">
          <div className="sticky-scroll flex max-h-40 flex-wrap gap-2 overflow-y-auto pr-2 md:max-h-60">
            {popTags.map((tag) => (
              <button key={tag.id_url} onClick={() => toggleTag(tag)} className={`cursor-pointer rounded border px-2 py-1 text-[9px] font-medium tracking-wider whitespace-nowrap uppercase transition-all ${
            ...
            {tags.filter((tag) => selectedTags.includes(tag) && !popTags.some((p) => p.id_url === tag.id_url)).map((tag) => (
                <button key={tag.id_url} onClick={() => toggleTag(tag)} className="bg-archive-accent border-archive-accent cursor-pointer rounded border px-2 py-1 text-[9px] font-medium tracking-wider whitespace-nowrap text-white uppercase transition-all" >
                  {getLocalizedTagName(tag)}
                </button>
              ))}
          </div>
          ...
          <div className="relative">
            <input type="text" placeholder={t("filter.search_tags")} className="archive-filter-input pr-9" value={tagQuery} onChange={(e) => setTagQuery(e.target.value)} />
            <svg className="pointer-events-none absolute top-1/2 right-3 h-3.5 w-3.5 -translate-y-1/2 opacity-25" fill="none" stroke="currentColor" viewBox="0 0 24 24" >
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            {tagQuery.trim().length > 0 && (
              <ul className="border-archive-ink/10 dark:border-archive-ink-dark/10 absolute right-0 left-0 z-10 overflow-hidden rounded-xl border bg-white shadow-lg dark:bg-neutral-900">
                {tags.filter((tag) => tag.names.some((tn) => tn.name.toLowerCase().includes(tagQuery.toLowerCase()))).map((tag) => (
                    <li key={tag.id_url} onMouseDown={() => { toggleTag(tag); setTagQuery(""); }} className="hover:bg-archive-accent cursor-pointer px-4 py-2 text-[11px] font-medium transition-colors hover:text-white">
                      {getLocalizedTagName(tag)}
                    </li>
                  ))}
              </ul>
      ...
      <FilterCard title={t("filter.halls")}>
        <div className="space-y-3">
          {Object.keys(hallMap).map((hall) => (
            <label key={hall} className="group flex cursor-pointer items-center space-x-3">
              <input type="checkbox" checked={selectedHalls.includes(hallMap[hall])} onChange={() => toggleHall(hallMap[hall])} className="border-archive-ink/20 text-archive-accent focus:ring-archive-accent cursor-pointer rounded" />
        ...
        <div>
          <div className="relative" ref={artistInputRef}>
            ...
            {filteredArtists.length > 0 && (
              <ul className={`border-archive-ink/10 dark:border-archive-ink-dark/10 absolute right-0 left-0 z-10 overflow-hidden rounded-xl border bg-white shadow-lg dark:bg-neutral-900 ${ dropdownAbove ? "bottom-full mb-1" : "top-full mt-1"}`}>
                {filteredArtists.map((artist) => (
                  <li key={artist} onMouseDown={() => selectArtist(artist)} className="hover:bg-archive-accent cursor-pointer px-4 py-2 text-[11px] font-medium transition-colors hover:text-white" >
                    {artist}
                  </li>
                ))}
              </ul>
          ...
          {selectedArtists.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {selectedArtists.map((artist) => (
                <span key={artist} className="bg-archive-accent inline-flex items-center gap-1.5 rounded px-2.5 py-1 text-[9px] font-medium tracking-wider text-white uppercase">
                  {artist}
                  <button onClick={() => removeArtist(artist)} className="leading-none opacity-70 transition-opacity hover:opacity-100">
