...
async def get_productions(request: Request, db: Session = Depends(get_db), cursor: int | None = Query(None), 
                          limit: int = Query(20, ge=1, le=100)) -> ProductionListResponse:
...
async def get_production(production_id: int, request: Request, db: Session = Depends(get_db),
                         language: str | None = Depends(get_accepted_language)) -> ProductionResponse:
    base_url = str(request.base_url).rstrip("/")
    try:
