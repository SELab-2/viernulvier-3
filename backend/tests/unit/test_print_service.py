...
class TestBuildPrintResponse:
    ...
    def test_all_fields_present(self):
        from src.services.print_service import build_print_response
        p = _make_print(id=1, title="Affiche", description="Seizoen 2024", print_type="poster")
        result = build_print_response(p, BASE_URL)
...
class TestUploadPrint:
    def _make_db(self):
        ...
        db.refresh.side_effect = lambda obj: (
            setattr(obj, "id", 1) or
            setattr(obj, "uploaded_at", datetime(2024, 1, 1, tzinfo=timezone.utc))
        )
        return db
    ...
    def test_object_key_format(self):
        ...
        upload_print(
            db=db, filename="affiche.pdf", content_type="application/pdf",
            data=b"%PDF", minio_client=minio, base_url=BASE_URL,
        )
    ...
    def test_title_and_description_stored(self):
        ...
        upload_print(
            db=db, filename="x.pdf", content_type="application/pdf",
            data=b"%PDF", minio_client=minio, base_url=BASE_URL,
...
class TestDeletePrint:
    ...
    def test_reraises_other_s3_errors(self):
        ...
        err = S3Error(
            code="InternalError", message="", resource="", request_id="",
            host_id="", response=MagicMock()
        )
        minio.remove_object.side_effect = err
