...
def create_user_and_login(
    client: TestClient, db_session: Session, username: str, permissions=None, language: str | None = None):
    password = "test_pw"
...
def test_get_productions_success(client: TestClient, db_session: Session, many_productions):
    ...
    response = client.get(
        BASE_URL + "/",
        headers = headers,
        params={"cursor": next_cursor, "limit": 5}
    )
    assert response.status_code == 200
...
def test_get_production_by_id_all_infos(client: TestClient, db_session: Session, productions_limited):
    headers = create_user_and_login(client, db_session, "normal_user")
...
def test_get_production_by_id_valid_language(client: TestClient, db_session: Session, productions_limited):
...
def test_get_production_by_id_invalid_language(client: TestClient, db_session: Session, productions_limited):
...
def test_patch_production_add_info_success(client: TestClient, db_session: Session, language_en, productions_limited):
...
def test_patch_production_add_info_invalid_language(client: TestClient, db_session: Session, language_en, productions_limited):
    headers = create_user_and_login(client, db_session, "update_production_user", [Permissions.ARCHIVE_UPDATE])
    production = productions_limited[1]
...
def test_patch_production_delete_info_success(client: TestClient, db_session: Session, language_en, productions_limited):
    headers = create_user_and_login(client, db_session, "update_production_user", [Permissions.ARCHIVE_UPDATE])
    production = productions_limited[0]
...
def test_delete_production_success(client: TestClient, db_session: Session, productions_limited):
    headers = create_user_and_login(client, db_session, "delete_production_user", [Permissions.ARCHIVE_DELETE])    
...
def test_delete_production_failure(client: TestClient, db_session: Session, productions_limited):
...
def test_delete_production_not_found(client: TestClient, db_session: Session, productions_limited):
    headers = create_user_and_login(client, db_session, "delete_production_user", [Permissions.ARCHIVE_DELETE])    
